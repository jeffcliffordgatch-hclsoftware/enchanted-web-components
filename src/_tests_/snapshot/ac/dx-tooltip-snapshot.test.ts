/*
 ********************************************************************
 * Licensed Materials - Property of HCL                             *
 *                                                                  *
 * Copyright HCL Technologies Ltd. 2025. All Rights Reserved.       *
 *                                                                  *
 * Note to US Government Users Restricted Rights:                   *
 *                                                                  *
 * Use, duplication or disclosure restricted by GSA ADP Schedule    *
 ********************************************************************
 */
// External imports
import { $, browser } from '@wdio/globals';
import { html, render } from 'lit';

// Component imports
import '../../../components/ac/dx-tooltip';
import '../../../components/ac/dx-button';

// Helper imports
import { TOOLTIP_PLACEMENT, TOOLTIP_TYPE, TOOLTIP_VARIANT } from '../../../types/cssClassEnums';
import { appendEnchantedStylingLink, SNAPSHOT_WINDOW_HEIGHT, SNAPSHOT_WINDOW_WIDTH } from '../utils';

function renderSmallSingleHtml() {
  return html`
    <div data-testid="dx-tooltip-layout" style="display: flex; flex-direction: column; gap: 40px;">
      <div style="display: flex; justify-content: space-between;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Top Start</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_TOP_START}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Top</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_TOP}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Top End</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_TOP_END}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
      </div>
      <div style="display: flex; justify-content: space-between;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Bottom Start</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_BOTTOM_START}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Bottom</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_BOTTOM}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Bottom End</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_BOTTOM_END}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
      </div>
      <div style="display: flex; justify-content: space-between;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Right Start</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_RIGHT_START}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Right</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_RIGHT}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Right End</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_RIGHT_END}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
      </div>
      <div style="display: flex; justify-content: space-between;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Left Start</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_LEFT_START}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Left</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_LEFT}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Left End</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_LEFT_END}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
      </div>
    </div>
  `;
}

function renderMediumSingleHtml() {
  return html`
    <div data-testid="dx-tooltip-layout" style="display: flex; flex-direction: column; gap: 40px;">
      <div style="display: flex; justify-content: space-between;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Top Start</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_TOP_START}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Top</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_TOP}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Top End</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_TOP_END}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
      </div>
      <div style="display: flex; justify-content: space-between;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Bottom Start</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_BOTTOM_START}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Bottom</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_BOTTOM}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Bottom End</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_BOTTOM_END}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
      </div>
      <div style="display: flex; justify-content: space-between;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Right Start</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_RIGHT_START}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Right</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_RIGHT}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Right End</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_RIGHT_END}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
      </div>
      <div style="display: flex; justify-content: space-between;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Left Start</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_LEFT_START}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Left</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_LEFT}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Left End</label>
          <dx-tooltip
            show=true
            tooltiptext="Tooltip"
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_SINGLE_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_LEFT_END}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
      </div>
    </div>
  `;
}

function renderSmallMultiHtml() {
  return html`
    <div data-testid="dx-tooltip-layout" style="display: flex; flex-direction: column;">
      <div style="display: flex; flex-direction: column; justify-content: space-between;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Top Start</label>
          <dx-tooltip
            show=true
            tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_TOP_START}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Top</label>
          <dx-tooltip
            show=true
            tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_TOP}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 40px;">Tooltip Top End</label>
          <dx-tooltip
            show=true
            tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_TOP_END}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
      </div>
      <div style="display: flex; flex-direction: column; justify-content: space-between;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label>Tooltip Bottom Start</label>
          <dx-tooltip
            show=true
            tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_BOTTOM_START}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-top: 40px;">Tooltip Bottom</label>
          <dx-tooltip
            show=true
            tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_BOTTOM}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-top: 40px;">Tooltip Bottom End</label>
          <dx-tooltip
            show=true
            tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_BOTTOM_END}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
      </div>
      <div style="display: flex; justify-content: center;">
        <div style="display: flex; flex-direction: column; justify-content: space-between; margin-right: 10px;">
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin-top: 40px;">Tooltip Left Start</label>
            <dx-tooltip
              show=true
              tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
              tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
              tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
              placement=${TOOLTIP_PLACEMENT.TOOLTIP_LEFT_START}
            >
              <dx-button slot="target" buttontext="Button"></dx-button>
            </dx-tooltip>
          </div>
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin: 10px 0px;">Tooltip Left</label>
            <dx-tooltip
              show=true
              tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
              tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
              tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
              placement=${TOOLTIP_PLACEMENT.TOOLTIP_LEFT}
            >
              <dx-button slot="target" buttontext="Button"></dx-button>
            </dx-tooltip>
          </div>
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin: 10px 0px;">Tooltip Left End</label>
            <dx-tooltip
              show=true
              tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
              tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
              tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
              placement=${TOOLTIP_PLACEMENT.TOOLTIP_LEFT_END}
            >
              <dx-button slot="target" buttontext="Button"></dx-button>
            </dx-tooltip>
          </div>
        </div>
        <div style="display: flex; flex-direction: column; justify-content: space-between;">
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin-top: 40px;">Tooltip Right Start</label>
            <dx-tooltip
              show=true
              tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
              tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
              tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
              placement=${TOOLTIP_PLACEMENT.TOOLTIP_RIGHT_START}
            >
              <dx-button slot="target" buttontext="Button"></dx-button>
            </dx-tooltip>
          </div>
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin: 10px 0px;">Tooltip Right</label>
            <dx-tooltip
              show=true
              tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
              tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
              tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
              placement=${TOOLTIP_PLACEMENT.TOOLTIP_RIGHT}
            >
              <dx-button slot="target" buttontext="Button"></dx-button>
            </dx-tooltip>
          </div>
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin: 10px 0px;">Tooltip Right End</label>
            <dx-tooltip
              show=true
              tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
              tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_SMALL}
              tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
              placement=${TOOLTIP_PLACEMENT.TOOLTIP_RIGHT_END}
            >
              <dx-button slot="target" buttontext="Button"></dx-button>
            </dx-tooltip>
          </div>
        </div>
      </div>
    </div>
  `;
}

function renderMediumMultiHtml() {
  return html`
    <div data-testid="dx-tooltip-layout" style="display: flex; flex-direction: column;">
      <div style="display: flex; flex-direction: column; justify-content: space-between;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 50px;">Tooltip Top Start</label>
          <dx-tooltip
            show=true
            tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_TOP_START}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 50px;">Tooltip Top</label>
          <dx-tooltip
            show=true
            tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_TOP}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-bottom: 50px;">Tooltip Top End</label>
          <dx-tooltip
            show=true
            tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_TOP_END}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
      </div>
      <div style="display: flex; flex-direction: column; justify-content: space-between;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label>Tooltip Bottom Start</label>
          <dx-tooltip
            show=true
            tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_BOTTOM_START}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-top: 50px;">Tooltip Bottom</label>
          <dx-tooltip
            show=true
            tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_BOTTOM}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <label style="margin-top: 50px;">Tooltip Bottom End</label>
          <dx-tooltip
            show=true
            tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
            tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
            tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
            placement=${TOOLTIP_PLACEMENT.TOOLTIP_BOTTOM_END}
          >
            <dx-button slot="target" buttontext="Button"></dx-button>
          </dx-tooltip>
        </div>
      </div>
      <div style="display: flex; justify-content: center;">
        <div style="display: flex; flex-direction: column; justify-content: space-between; margin-right: 10px;">
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin-top: 50px;">Tooltip Left Start</label>
            <dx-tooltip
              show=true
              tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
              tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
              tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
              placement=${TOOLTIP_PLACEMENT.TOOLTIP_LEFT_START}
            >
              <dx-button slot="target" buttontext="Button"></dx-button>
            </dx-tooltip>
          </div>
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin: 20px 0px 10px;">Tooltip Left</label>
            <dx-tooltip
              show=true
              tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
              tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
              tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
              placement=${TOOLTIP_PLACEMENT.TOOLTIP_LEFT}
            >
              <dx-button slot="target" buttontext="Button"></dx-button>
            </dx-tooltip>
          </div>
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin: 10px 0px 20px;">Tooltip Left End</label>
            <dx-tooltip
              show=true
              tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
              tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
              tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
              placement=${TOOLTIP_PLACEMENT.TOOLTIP_LEFT_END}
            >
              <dx-button slot="target" buttontext="Button"></dx-button>
            </dx-tooltip>
          </div>
        </div>
        <div style="display: flex; flex-direction: column; justify-content: space-between;">
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin-top: 50px;">Tooltip Right Start</label>
            <dx-tooltip
              show=true
              tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
              tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
              tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
              placement=${TOOLTIP_PLACEMENT.TOOLTIP_RIGHT_START}
            >
              <dx-button slot="target" buttontext="Button"></dx-button>
            </dx-tooltip>
          </div>
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin: 20px 0px 10px;">Tooltip Right</label>
            <dx-tooltip
              show=true
              tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
              tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
              tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
              placement=${TOOLTIP_PLACEMENT.TOOLTIP_RIGHT}
            >
              <dx-button slot="target" buttontext="Button"></dx-button>
            </dx-tooltip>
          </div>
          <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
            <label style="margin: 10px 0px 20px;">Tooltip Right End</label>
            <dx-tooltip
              show=true
              tooltiptext="Multiline tooltip. This should be set to a fixed width that is easy to quickly scan or read."
              tooltipSize=${TOOLTIP_VARIANT.TOOLTIP_MEDIUM}
              tooltipType=${TOOLTIP_TYPE.TOOLTIP_MULTI_LINE}
              placement=${TOOLTIP_PLACEMENT.TOOLTIP_RIGHT_END}
            >
              <dx-button slot="target" buttontext="Button"></dx-button>
            </dx-tooltip>
          </div>
        </div>
      </div>
    </div>
  `;
}

describe('DxTooltip - Snapshot testing', () => {
  before(async () => {
    await browser.setWindowSize(SNAPSHOT_WINDOW_WIDTH, SNAPSHOT_WINDOW_HEIGHT);
  });

  it('DxTooltip - should capture snapshot for all possible placement of a SMALL and SINGLE-LINE Tooltip component', async () => {
    const link = appendEnchantedStylingLink();

    render(renderSmallSingleHtml(), document.body);

    // Resize a bit to trigger tooltip update (fix for some intermittent issue)
    const originalSize = await browser.getWindowSize();
    await browser.setWindowSize(originalSize.width + 1, originalSize.height);
    // Restore original window size
    await browser.setWindowSize(originalSize.width, originalSize.height);

    let buttonLayout = await $('>>>div[data-testid="dx-tooltip-layout"]');
    await browser.checkElement(buttonLayout, 'dx-tooltip-snapshot-small-single-baseline-authoring');
    document.head.removeChild(link);
  });

  it('DxTooltip - should capture snapshot for all possible placement of a MEDIUM and SINGLE-LINE Tooltip component', async () => {
    const link = appendEnchantedStylingLink();

    render(renderMediumSingleHtml(), document.body);
    let buttonLayout = await $('>>>div[data-testid="dx-tooltip-layout"]');
    await browser.checkElement(buttonLayout, 'dx-tooltip-snapshot-medium-single-baseline-authoring');
    document.head.removeChild(link);
  });

  it('DxTooltip - should capture snapshot for all possible placement of a SMALL and MULTI-LINE Tooltip component', async () => {
    const link = appendEnchantedStylingLink();

    render(renderSmallMultiHtml(), document.body);
    let buttonLayout = await $('>>>div[data-testid="dx-tooltip-layout"]');
    await browser.checkElement(buttonLayout, 'dx-tooltip-snapshot-small-multi-baseline-authoring');
    document.head.removeChild(link);
  });

  it('DxTooltip - should capture snapshot for all possible placement of a MEDIUM and MULTI-LINE Tooltip component', async () => {
    const link = appendEnchantedStylingLink();

    render(renderMediumMultiHtml(), document.body);
    let buttonLayout = await $('>>>div[data-testid="dx-tooltip-layout"]');
    await browser.checkElement(buttonLayout, 'dx-tooltip-snapshot-medium-multi-baseline-authoring');
    document.head.removeChild(link);
  });
});
