...
export class DxAccordion extends DxAcBaseElement {
  ...
  render() {
    return html`
      <div
        part=${this.isLTR
          ? `${ACCORDION_PARTS.DX_ACCORDION_CONTAINER}`
          : `${ACCORDION_PARTS.DX_ACCORDION_CONTAINER_RTL}`}
      >
        <div
          part=${this.isLTR
            ? `${ACCORDION_PARTS.DX_ACCORDION_HEADER_SCSS}`
            : `${ACCORDION_PARTS.DX_ACCORDION_HEADER_SCSS_RTL}`}
        >
          ${this.showCheckbox
          ...
          <div
            part=${this.isLTR
              ? `${ACCORDION_PARTS.DX_ACCORDION_LABEL_COLUMN}`
              : `${ACCORDION_PARTS.DX_ACCORDION_LABEL_COLUMN_RTL}`}
            role="button"
            tabindex="-1"
          ...
          >
            ${this.label
              ? html`<div
                  part=${this.isLTR
                    ? `${ACCORDION_PARTS.DX_ACCORDION_LABEL_TEXT}`
                    : `${ACCORDION_PARTS.DX_ACCORDION_LABEL_TEXT_RTL}`}
                >
                  ${this.label}
                </div>`
        ...
        <span
          part=${this.isLTR
            ? `${ACCORDION_PARTS.DX_ACCORDION_ARROW}`
            : `${ACCORDION_PARTS.DX_ACCORDION_ARROW_RTL}`}
          role="button"
          tabindex="0"
        ...
        >
          <icon-chevron-down
            part=${this.isLTR
              ? `${ACCORDION_PARTS.DX_ACCORDION_ARROW_ICON}`
              : `${ACCORDION_PARTS.DX_ACCORDION_ARROW_ICON_RTL}`}
            size="16"
          ></icon-chevron-down>
        </span>
      </div>
      ${this.open
        ? html`
            <div
              part=${this.isLTR
                ? `${ACCORDION_PARTS.DX_ACCORDION_CONTENT}`
                : `${ACCORDION_PARTS.DX_ACCORDION_CONTENT_RTL}`}
            >
              <slot
